...
export default function BookingModal() {
  ...
  const inputStyles = "w-full bg-transparent border-b border-slate-300 dark:border-slate-700 pb-2 text-[14px] text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-[#F2306D] dark:focus:border-[#F2306D] transition-colors rounded-none appearance-none";
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-4xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden my-auto border border-slate-100 dark:border-slate-800"
        >
          {/* Close Button */}
          <button 
            onClick={handleClose}
          ...
          <div className="p-8 sm:p-12">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-8 text-center sm:text-left">Book an Appointment</h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-10">
              ...
              <div className="flex flex-col md:flex-row items-end gap-8 md:gap-12">
                ...
                <div className="w-full md:w-1/2 flex flex-col gap-2">
                  <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wider mb-2">Time</span>
              ...
              <div className="w-full">
                <textarea 
                  ...
                  rows={4}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4 text-[14px] text-slate-800 dark:text-slate-200 placeholder:text-slate-400 focus:outline-none focus:border-[#F2306D] focus:ring-1 focus:ring-[#F2306D] transition-colors rounded-xl resize-none"
                  required
                ></textarea>
              </div>
              ...
              <div className="text-center sm:text-left mt-2">
                <button 
                  type="submit"
                  className="px-10 py-3.5 rounded-full bg-[#F2306D] hover:bg-[#D9265F] text-white text-[13px] font-bold tracking-wide uppercase transition-colors shadow-sm w-full sm:w-auto"
                >
                  Request Appointment
                </button>
              </div>
